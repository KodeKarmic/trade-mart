...
/**
 * Simple implementation of {@link TradeVersionValidator} used by the service
 * when
 * no custom validator is provided. Rules:
 * - if existing == null -> accept
 * - if incoming.version == null -> accept
 * - if incoming.version < existing.version -> reject
 * - otherwise accept
 */
@Component
public class SimpleTradeVersionValidator implements TradeVersionValidator {
  ...
  public void validate(TradeDto incoming, TradeEntity existing) throws TradeRejectedException {
    ...
    if (inVer == null)
      return; // accept null incoming version
